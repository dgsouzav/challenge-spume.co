...
bootloader.dependencies = [
     ...
     * the system boot
     */
    "theme", "splash", "home", "article_tile"
];
...
bootloader.loadComponents.add(NULL => {
    ...
    app.post("themes", { theme: app.theme_name }).then(theme => {
        ...
        if(!APP_NEEDS_LOGIN||app.hash) app.load("splash.htm").then(NULL => {
            ...
            app.call("components/article_tile.htm").then(article_tile => {
                app.components.article_tile = article_tile.data.prepare(app.colors())
                bootloader.ready("article_tile")
            })
